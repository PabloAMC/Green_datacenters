...
*   **`datacenter_lcoe.py`**: The core simulation and optimization script (v5.3). Runs the 8760-hour dispatch, vectorizes the scenario grid, solves the 3D optimization via multi-start Nelder-Mead, and exposes a CLI for region/workload scenarios plus a flexibility sensitivity sweep.
*   **`model_documentation.md`**: The technical reference — all mathematical formulations (Wright's Law learning curves, Gaussian-copula wind-solar coupling, synoptic Dunkelflaute factor, battery Wöhler degradation, CCGT/OCGT sizing, carbon trajectories, the economic-shed rule) with data sources and an accuracy summary.
